...
struct block_data
{
    ...
    char path_to_exec[2048];
};

struct block_data blocked_execs[CRITICAL_NUM];

int is_access_deny(char* file, char* app)
{
    for (int i = 0; i < CRITICAL_NUM; i++)
    {
        if ((!strcmp(app, blocked_execs[i].path_to_exec)) && (!strcmp(file, blocked_execs[i].path_to_file)))
        {
            return 1;
        }
    }
...
void parse_conf_data_file()
{
    for (int i = 0; i < CRITICAL_NUM; i++)
    {
        for (int k = 0; k < 2048; k++)
        {
            blocked_execs[i].path_to_exec[k] = '\0';
            blocked_execs[i].path_to_file[k] = '\0';
        }
    }
    NTSTATUS status = STATUS_SUCCESS;
    HANDLE fileObject;
    UNICODE_STRING unicodePath;
    RtlInitUnicodeString(&unicodePath, L"\\Device\\HarddiskVolume3\\lab_4_driver\\config_file.txt");
    
    DbgPrint("\\??\\C:\\lab_4_driver\\config_file.txt");
    OBJECT_ATTRIBUTES objAttr;
    InitializeObjectAttributes(&objAttr, &unicodePath,
    ...
    if (!NT_SUCCESS(status)) {
        DbgPrint("Ошибка открытия файла 0x%x\n",t);
        ZwClose(fileObject);
        return;
    }
    ...
    while (TRUE) {
        ...
        if (!NT_SUCCESS(status)) {
            ...
            {
                DbgPrint("Достигнут конец файла\n");
                break;
            }
            else
            {
        ...
        while (buffer[i] != ' ')
        {
            blocked_execs[j].path_to_exec[i] = buffer[i];
            buffer[i] = '\0';
            i++;
            
            byteOffset.QuadPart++;
        }
        blocked_execs[j].path_to_exec[i] = '\0';
        int k = 0;
        byteOffset.QuadPart++;
        buffer[i] = '\0';
        i++;
        while (buffer[i] != '\n' && buffer[i] != '\0')
        {
            blocked_execs[j].path_to_file[k] = buffer[i];
            buffer[i] = '\0';
            i++;
            k++;
            byteOffset.QuadPart++;
        }
        blocked_execs[j].path_to_file[k-1] = '\0';
        DbgPrint("%s ||| %s %d %d %d\n", blocked_execs[j].path_to_exec, blocked_execs[j].path_to_file, blocked_execs[j].path_to_file[k],
            blocked_execs[j].path_to_file[k-1], blocked_execs[j].path_to_file[k - 2]);
        j++;
        byteOffset.QuadPart++;
        buffer[i] = '\0';
    }
...
FLT_PREOP_CALLBACK_STATUS
PtPreOperationPassThrough_Read
(
    _Inout_ PFLT_CALLBACK_DATA Data,
    _In_ PCFLT_RELATED_OBJECTS FltObjects,
    _Flt_CompletionContext_Outptr_ PVOID* CompletionContext
...
PtPostOperationPassThrough_Write(
    _Inout_ PFLT_CALLBACK_DATA Data,
    _In_ PCFLT_RELATED_OBJECTS FltObjects,
...
FLT_PREOP_CALLBACK_STATUS
PtPreOperationPassThrough_Write
(
    _Inout_ PFLT_CALLBACK_DATA Data,
    _In_ PCFLT_RELATED_OBJECTS FltObjects,
    _Flt_CompletionContext_Outptr_ PVOID *CompletionContext
...
--*/
{
    ...
    DbgPrint("HI IT IS KERNEL MODE DRIVER!");
    PT_DBG_PRINT( PTDBG_TRACE_ROUTINES,
...
)
{
    ...
    PT_DBG_PRINT(PTDBG_TRACE_ROUTINES,
        ("PassThrough!File_Create_Callback: Entered\n"));
...
)
{
    ...
    NTSTATUS status = FltGetFileNameInformation(
        ...
        FLT_FILE_NAME_OPENED,  // формат имени файла
        &nameInfo           // результат
    );
    char path[] = "\\Device\\HarddiskVolume3\\lab_4_driver\\config_file.txt";
    if (NT_SUCCESS(status)) {
        // Работа с именем файла
        UNICODE_STRING fileName = nameInfo->Name;
        int i = 0;
        int k = 0;
        while (fileName.Buffer[i] != '\0' && path[i] != '\0')
        {
            if (fileName.Buffer[i] != path[i])
            {
                k = i;
                break;
            }
...
)
{
    ...
    char filename[2048];
    sprintf(filename, "%S", fileName.Buffer);
    ...
    if (!thread)
    {
        DbgPrint("Invalid thread object\n");
    ...
    if (!process)
    {
        DbgPrint("Failed to get process object\n");
    ...
    DbgPrint("Process ID: 0x%x, Thread ID: 0x%x\n", processId, threadId);
    PUNICODE_STRING file_open;
    PEPROCESS test_check;
    HANDLE one_test = PsGetCurrentProcessId();
    if (one_test == NULL) return FLT_PREOP_SUCCESS_WITH_CALLBACK;

    status = PsLookupProcessByProcessId(one_test, &test_check);
    if (!NT_SUCCESS(status)) return FLT_PREOP_SUCCESS_WITH_CALLBACK;

    status = SeLocateProcessImageName(process, &file_open);
    if (!NT_SUCCESS(status)) return FLT_PREOP_SUCCESS_WITH_CALLBACK;

    char process_char_name[2048];
    sprintf(process_char_name, "%S", file_open->Buffer);
    DbgPrint("Test get name: %s\n", process_char_name);
    if (is_access_deny(filename, process_char_name))
    {
